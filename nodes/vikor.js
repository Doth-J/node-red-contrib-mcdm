    ...
    function VIKORNode(config) {
        ...
        this.on('input', (msg, send, done) => {
            ...
            decisionMatrix.forEach((_, i) => {
                if (!Array.isArray(_))
                    throw Error(`Dimension Error: Decision matrix must be a 2d array, check alternative no.${i + 1}`);
                if (_.length != weights.length)
                    throw Error(`Dimension Mismatch: Decision matrix must be same length as weight, check alternative no.${i + 1}`);
            });
            if (config.performance)
                performance.clearMarks(), performance.clearMeasures(), performance.mark("start");
            const squaredColumnSums = utils_1.default.squareRoot(utils_1.default.sumColumns(utils_1.default.square2D(decisionMatrix))), normalizedMatrix = utils_1.default.divideArrays(decisionMatrix, squaredColumnSums), weightedNormalizedMatrix = normalizedMatrix.map((row, i) => row.map(val => val * weights[i]));
            if (config.performance)
            ...
            const S = weightedNormalizedMatrix.map(alt => PIS.reduce((sum, pisVal, idx) => sum + pisVal - alt[idx], 0)), R = weightedNormalizedMatrix.map(alt => PIS.reduce((max, pisVal, idx) => Math.max(max, pisVal - alt[idx]), 0));
            if (config.performance)
                performance.mark("step 3");
            const S_min = Math.min(...S), S_max = Math.max(...S), R_min = Math.min(...R), R_max = Math.max(...R), Q = S.map((S_val, i) => v * (S_val - S_min) / (S_max - S_min) + (1 - v) * (R[i] - R_min) / (R_max - R_min)), bestAlternativeIndex = Q.indexOf(Math.min(...Q));
            if (config.performance)
