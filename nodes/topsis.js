...
module.exports = function (RED) {
    function TOPSISNode(config) {
        ...
        this.on('input', (msg, send, done) => {
            ...
            const weightedRowSums = weightedMatrix.map((row) => utils_1.default.sum(row)), PIS = weightedMatrix[weightedRowSums.indexOf(config.maximize ? Math.max(...weightedRowSums) : Math.min(...weightedRowSums))], NIS = weightedMatrix[weightedRowSums.indexOf(!config.maximize ? Math.max(...weightedRowSums) : Math.min(...weightedRowSums))];
            if (config.performance)
                performance.mark("step 2");
            const distnancePIS = utils_1.default.squareRoot(utils_1.default.sumRows(utils_1.default.square2D(utils_1.default.subtractArrays(weightedMatrix, PIS)))), distnanceNIS = utils_1.default.squareRoot(utils_1.default.sumRows(utils_1.default.square2D(utils_1.default.subtractArrays(weightedMatrix, NIS))));
            if (config.performance)
